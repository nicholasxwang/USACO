...
import java.util.*;
class Tractor {
    public static int twoD_to_oneD(int x, int y, int size_x, int size_y){ return x*size_y + y;}
    public static ArrayList<Integer> oneD_to_twoD(int id, int size_x, int size_y){
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i<max_x; i++){
            for (int j = 0; j<max_y; j++){
                int v = twoD_to_oneD(i, j, max_x, max_y);
        ...
        for (int i = 0; i<processed_points.size(); i++){
            for (int j = 0; j<processed_points.size(); j++){
                ArrayList<Integer> xy = oneD_to_twoD(processed_points.get(i), max_x, max_y);
                x = xy.get(0);
                y = xy.get(1);
                ArrayList<Integer> xy2 = oneD_to_twoD(processed_points.get(j), max_x, max_y);
                int x2 = xy2.get(0);
                int y2 = xy2.get(1);
                int value = 9;
                if (i == j){
